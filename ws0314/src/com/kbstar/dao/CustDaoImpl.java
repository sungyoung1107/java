...
//		System.out.println("Driver Loading 성공하였습니다...");
	...
	public void insert(Cust v) throws Exception {
		try (Connection conn = getConnection(); PreparedStatement pstmt = conn.prepareStatement(Sql.insertSql);) {
...
//			e1.printStackTrace();
	...
	public List<Cust> selectAll() throws Exception {
		...
		try (Connection con = getConnection(); PreparedStatement pstmt = con.prepareStatement(Sql.selectAllsSql);) {
			try (ResultSet rset = pstmt.executeQuery();) {
				while (rset.next()) {
					Cust cust = null;
